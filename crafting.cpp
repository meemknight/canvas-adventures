...
///quick buttons
   ...
   // changetexture(inventory_meniu_sprite,inventory_sprites::grey,0);
    ...
    ///draw items
        ...
        if(leftreleased&&getbutton(pointingto)==-7){pedding=7;}   ///quick buttons
        if(leftreleased&&getbutton(pointingto)==-8){pedding=34;}
        if(leftreleased&&getbutton(pointingto)==-9){pedding=43;}
        if(leftreleased&&getbutton(pointingto)==-10){pedding=59;}
