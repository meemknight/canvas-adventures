   ...
   if(rec.type)
        ...
        if(rec.number>1)
           ...
           // text.scale(1,1);
                ...
                if(item->getarmour()||item->getfalldamagerezistance())
                    {
                            text.setString("Armour: ");
                            text.setString(text.getString()+inttostring(item->getarmour()));
                            text.setPosition(13*64+xview,2.5*64+yview);
                            window.draw(text);
                            if(item->getfalldamagerezistance()>=0)
                                {
                                text.setString("Fall damage reduction: ");
                                text.setString(text.getString()+inttostring(item->getfalldamagerezistance()));
                                }else
                                {
                                text.setString("Provides immunity fall damage");
                                }
                            text.setPosition(13*64+xview,3.0*64+yview);
                            window.draw(text);
