...
{                                                                                       /// !!!
      uint8_t numberofframes=7;//sprite.getTexture()->getSize().x/sizeofentity.x;
                                                                                        /// seg fault !!!
    if(numberofframes!=1){
...
bool entities::dealfalldamage(float damage)
{
    ...
    if(falldamagerezistance<=-1){damage=0;}
    life-=damage;
    if(damage==0){return 0;}else{return 1;}
