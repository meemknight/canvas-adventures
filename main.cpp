...
#ifdef DEBUG
   window.create(sf::VideoMode(WIDE, HEIGHT), "Keke terraria!",sf::Style::Titlebar|sf::Style::Close|sf::Style::Resize);
#endif // DEBUG
sf::ContextSettings setting;


#ifdef RELEASE
  window.create(sf::VideoMode(WIDE, HEIGHT), "Keke terraria!",sf::Style::Titlebar|sf::Style::Close|sf::Style::Fullscreen);
#endif // RELEASE
                                ...
                                if(enemies[i]==player)
                                    {
                                        if(player.invurnerabilitytime<=0)
                                            {
                                            player.invurnerabilitytime=0.5;
                                            player.life-=enemies[i].attatc;
                                            if(enemies[i].type==et::ice_slime)
                                                {
                                                player.frosttime=3;
                                                }

                                         //cout<<player.life<<" "<<enemies[i].attatc<<endl;
...
for(uint8_t i=0;i<humanlike_enemies.size();i++)
    ...
    if(humanlike_enemies[i]==pointingto)
        ...
        if(!humanlike_enemies[i].invurnerabilitytime&&inventory[slot]->getattackpower()&&sf::Mouse::isButtonPressed(sf::Mouse::Left)
           ...
           &&attacktime.getElapsedTime().asSeconds()>inventory[slot]->getspeed()/2
            ...
            {
                attacktime.restart();
            humanlike_enemies[i].life-=inventory[slot]->getattackpower();
            humanlike_enemies[i].invurnerabilitytime=0.5;

            humanlike_enemies[i].frosttime=inventory[slot]->getfreeztime();
    ...
    if(humanlike_enemies[i].life<=0)                          /// kill enemy
        {
            droppedi.push_back(droppeditem(0,items::bone,humanlike_enemies[i].x,humanlike_enemies[i].y,1));
            humanlike_enemies[i].type=0;
            humanlike_enemies.erase(humanlike_enemies.begin()+i);
            //enemies
            //i--;
                                ...
                                if(humanlike_enemies[i]==player)
                                    {
                                        humanlike_enemies[i].animationstate=humanlikeenemy::attacking;
                                        if(player.invurnerabilitytime<=0)
                                            {
                                            player.invurnerabilitytime=0.5;
                                            player.life-=humanlike_enemies[i].attatc;
                                            if(humanlike_enemies[i].type==et::ice_slime)
                                                {
                                                player.frosttime=3;
                                                }
                ...
                if(isconsumable(inventory[slot]->type)&&!inventory[slot]->placeble())
                    {
                        ...
                        {
                            case items::apple:player.life+=15;inventory[slot]->number--;if(player.life>player.maxlife){player.life=player.maxlife;}break;
                            case items::healthbottle:player.life+=50;inventory[slot]->number--;if(player.life>player.maxlife){player.life=player.maxlife;}break;
